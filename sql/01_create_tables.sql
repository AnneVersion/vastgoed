...
CREATE TABLE IF NOT EXISTS betalingen (
    ...
    methode VARCHAR(50) DEFAULT 'iDEAL',
    bank VARCHAR(100),
...
-- geblokkeerde_datums (blocked dates for calendar)
CREATE TABLE IF NOT EXISTS geblokkeerde_datums (
    id SERIAL PRIMARY KEY,
    woning_id INTEGER REFERENCES woningen(id),
    datum DATE NOT NULL,
    reden VARCHAR(255),
    UNIQUE(woning_id, datum)
);
...
CREATE INDEX IF NOT EXISTS idx_geblokkeerde_datums_woning ON geblokkeerde_datums(woning_id, datum);

-- Trigger to auto-update updated_at on woningen
